  ...
  int n) 
{
  ...
  if(j < n && i < n){
    ...
    for(int k = 0; k < n; k++){
      sum0 += a[k * n + i] * b[j * n + k];
      sum1 += a[k * n + i] * b[(j + 1 * blockDim.y) * n + k];
      sum2 += a[k * n + i] * b[(j + 2 * blockDim.y) * n + k];
      sum3 += a[k * n + i] * b[(j + 3 * blockDim.y) * n + k];
    }
    c[i * n + j] = sum0;
    c[i * n + (j + 1 * blockDim.y)] = sum1;
    c[i * n + (j + 2 * blockDim.y)] = sum2;
    c[i * n + (j + 3 * blockDim.y)] = sum3;
  }
}
...
// Launch kernel
     int ty = 16; int tx = 16;
     dim3 threads(tx,ty);
     dim3 grid(ceil(n / threads.x), ceil((n / 4) / threads.y));
