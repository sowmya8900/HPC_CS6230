  ...
  int n) 
{
  ...
  __shared__ float abuf[16][16];
  __shared__ float bbuf[16][16];

  if(i < n && j < n){
    float sum = 0.0f;
    for(int ks = 0; ks < n; ks += 16){
      abuf[threadIdx.y][threadIdx.x] = a[(ks + threadIdx.y) * n + i];
      bbuf[threadIdx.y][threadIdx.x] = b[j * n + ks + threadIdx.x];
      __syncthreads();
      for(int k = 0; k < 16; k++)
        sum += abuf[k][threadIdx.x] * bbuf[threadIdx.y][k];
      __syncthreads();
    }
    c[i * n + j] = sum;
  }
